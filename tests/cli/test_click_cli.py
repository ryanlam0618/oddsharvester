...
class TestComboSummaryRendering:
    ...
    def test_all_combos_empty_still_prints_summary_and_exits_0(self, runner):
        """Fork semantics (570036f): every combo returning zero rows WITH zero failures is valid
        empty data (OddsPortal has nothing for those league/season combos), so the run still
        shows the breakdown but exits 0 — the error path is reserved for actual failures."""
        combo_stats = [
